...
def scanline_convert(polygons, i, screen, zbuffer, color, shading_type, intensity, constants, light_sources):
    p0 = polygons[i]
    p1 = polygons[i+1]
    p2 = polygons[i+2]

    if shading_type != "flat":
        color[0] = random.randrange(0, 256)
        color[1] = random.randrange(0, 256)
        color[2] = random.randrange(0, 256)
...
def draw_polygons( matrix, screen, zbuffer, color, constants = [], light_sources = [], shading_type = "" ):
    ...
    while point < len(matrix) - 2:
        normal = calc_normal(matrix, point)[:]
        if normal[2] > 0:
            if shading_type == "flat":
                color = total_light(normal, constants, light_sources)
            ...
            draw_line( int(matrix[point][0]),
                       ...
                       matrix[point+2][2],
                       screen, zbuffer, color)
            scanline_convert(matrix, point, screen, zbuffer, color, shading_type, intensity, constants, light_sources)
            color[0] = (color[0] + 20) % 256
            color[1] = (color[1] + 60) % 256
            color[2] = (color[2] + 120) % 256
        point += 3
